...
const NaoKuo = {
    ...
    try {
      if (!ipLoacation) {
        return new Promise((resolve, reject) => {
          ...
          var url = `https://apis.map.qq.com/ws/location/v1/ip?key=【填写你的KEY】&output=jsonp`;
          script.src = url;
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = NaoKuo.getDistance(填写你的经度, 填写你的纬度, ipStore.result.location.lng, ipStore.result.location.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
